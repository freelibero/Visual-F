...
const today = [
  { key: "门诊人次", val: 1286, prev: 1192, icon: "👤" },
  { key: "入院患者", val: 47, prev: 43, icon: "🏥" },
  { key: "手术台次", val: 23, prev: 26, icon: "🔪" },
  { key: "检查量", val: 312, prev: 298, icon: "📊" },
];
...
<template>
  <dv-border-box1 :style="{ width: '617px', height: '180px' }" :color="['#04c2f1', '#d2e0eeea']">
    <div class="stats-row">
      <div v-for="(s, i) in today" :key="s.key" class="stat-card" :class="{ in: anim }" :style="{ transitionDelay: i * 0.1 + 's' }">
        <div class="stat-line" />
        <div class="stat-body">
...
.delta-up { color: #91cc75; }
.delta-down { color: #ee6666; }
